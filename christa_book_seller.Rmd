...
# check that no customers are duplicated in training set
any(duplicated(train$id))

# train/test sets
book_train = train[seq(1, nrow(train), 2), ]
book_test = train[seq(2, nrow(train), 2), ]
...
hello <- aggregate(x=book[,10:38],by=list(book$id),FUN = sum)
...
book <- cbind(book,hello[,-1])

book$logPrice[book$logPrice == -Inf] <- 0

# merge features with training set
# merged_train <- merge(book_train,book,by="id")
# merged_test <- merge(book_train,book,by="id")
...
step(lm1)
# lm2 <- lm(formula = logtarg ~ totPrice + price.m + numOrd + 
#      `7` + `14` + `20` + `27` + `35` + `40` + `50`, 
#     data = merged[merged$logtarg != 0, -c(1, 41)])
